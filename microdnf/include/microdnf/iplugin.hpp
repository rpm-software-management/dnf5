...
/// Plugin API version
struct APIVersion {
    std::uint16_t major;  // microdnf and the plugin must implement the same `major` version to work together
    std::uint16_t minor;  // microdnf must implement the `minor` version >= than the plugin to work together
};
...
microdnf::APIVersion microdnf_plugin_get_api_version(void);
...
microdnf::PluginVersion microdnf_plugin_get_version(void);

/// Creates a new plugin instance. Passes the API version to the plugin.
microdnf::IPlugin * microdnf_plugin_new_instance(microdnf::APIVersion api_version);
