...
private:
    using TGetApiVersionFunc = decltype(&microdnf_plugin_get_api_version);
    using TGetNameFunc = decltype(&microdnf_plugin_get_name);
    using TGetVersionFunc = decltype(&microdnf_plugin_get_version);
    using TNewInstanceFunc = decltype(&microdnf_plugin_new_instance);
    using TDeleteInstanceFunc = decltype(&microdnf_plugin_delete_instance);
...
PluginLibrary::PluginLibrary(const std::string & library_path) : library(library_path) {
    get_api_version = reinterpret_cast<TGetApiVersionFunc>(library.get_address("microdnf_plugin_get_api_version"));
    get_name = reinterpret_cast<TGetNameFunc>(library.get_address("microdnf_plugin_get_name"));
    ...
    get_version = reinterpret_cast<TGetVersionFunc>(library.get_address("microdnf_plugin_get_version"));
    new_instance = reinterpret_cast<TNewInstanceFunc>(library.get_address("microdnf_plugin_new_instance"));
    delete_instance = reinterpret_cast<TDeleteInstanceFunc>(library.get_address("microdnf_plugin_delete_instance"));
...
void Plugins::register_plugin(std::unique_ptr<Plugin> && plugin) {
    ...
    logger->info(
        "Added microdnf plugin \"{}\" version \"{}.{}.{}\"", name, version.major, version.minor, version.micro);
}
...
void Plugins::load_plugin(const std::string & file_path) {
    ...
    auto version = iplugin->get_version();
    logger->info(
        "Loaded microdnf plugin \"{}\" (\"{}\") version \"{}.{}.{}\"",
        name,
        file_path,
        version.major,
        version.minor,
...
void Plugins::load_plugins(const std::string & dir_path) {
    ...
    for (const auto & path : lib_paths) {
        try {
            ...
            logger->error("Cannot load microdnf plugin \"{}\": {}", path.string(), ex.what());
    ...
    if (!failed_filenames.empty()) {
        throw std::runtime_error("Cannot load microdnf plugins: " + failed_filenames);
    }
}
