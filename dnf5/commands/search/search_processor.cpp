...
SearchResults SearchProcessor::get_results() {
    ...
    for (auto const & pattern : patterns) {
        ...
        pattern_matches |= get_name_matches(pattern);
        pattern_matches |= get_summary_matches(pattern);

        if (search_all) {
            pattern_matches |= get_description_matches(pattern);
            pattern_matches |= get_url_matches(pattern);
        }
        ...
        if (all_matches.empty() | search_all) {
    ...
    std::map<int, SearchPackages> priority_matches;
    ...
    // Just push the package groups into the final result with highest priorities first.
    SearchResults results;
    for (auto const & [priority, packages] : priority_matches | std::views::reverse) {
