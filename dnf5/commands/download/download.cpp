...
void DownloadCommand::configure() {
    ...
    if (srpm_option->get_value() && debugsource_option->get_value()) {
        throw libdnf5::cli::ArgumentParserConflictingArgumentsError(
            M_("Options \"--srpm\" and \"--debugsource\" are mutually exclusive"));
    }
...
void DownloadCommand::run() {
    ...
    if (debugsource_option->get_value()) {
        std::map<std::string, libdnf5::rpm::Package> debugsource_pkgs;
        ...
        for (auto & [nevra, pkg] : download_pkgs) {
            ...
            if (pkg_query.size() > 0) {
                for (const auto & dpkg : pkg_query) {
                    debugsource_pkgs.insert(create_nevra_pkg_pair(dpkg));
                }
            } else {
