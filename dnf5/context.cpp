...
public:
    ...
    bool repokey_remove(const libdnf5::rpm::KeyInfo & key_info) override {
        if (config->get_assumeno_option().get_value()) {
            return false;
        }

        std::cerr << libdnf5::utils::sformat(
                         _("The following PGP key (0x{}) has expired on {}:"),
                         key_info.get_short_key_id(),
                         libdnf5::utils::string::format_epoch(key_info.get_timestamp()))
                  << std::endl;
        for (auto & user_id : key_info.get_user_ids()) {
            std::cerr << libdnf5::utils::sformat(_(" UserID     : \"{}\"\n"), user_id);
        }
