...
/// Create the `--download-sort` and `--reverse` options for a command provided as an argument.
...
/// Throws `libdnf5::cli::ArgumentParserMissingDependentArgumentError` if `--reverse` was used without
/// `--download-sort` for the command provided as an argument. To be called from the command's `configure()`,
/// after argument parsing has completed.
DNF_API void check_download_order_options(dnf5::Command & command);
