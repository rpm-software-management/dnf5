...
void create_store_option(dnf5::Command & command) {
    ...
    store->set_description("Store the current transaction instead of running it.");
    ...
    store->set_parse_hook_func([&ctx](
                                   ...
                                   const char * value) {
        libdnf5::OptionString transaction_store_path(value);
        ctx.transaction_store_path = transaction_store_path.get_value_string();
        return true;
    });
}
