...
# Process input string using defined symbols and write the result to output
# file handle.
def process(input, output, symbols):
    ...
    while True:
        ...
        if conditions[0]:
            output.writelines(input[:token_found.start()])
        ...
        if token_found.group(1) in symbols:
            if conditions[0]:
                output.writelines(symbols[token_found.group(1)]["string"])
        input = input[token_found.end():]
        continue
...
def add_symbol_from_argument(argument):
    ...
    if value is None:
        ...
        boolean = True
    symbols[symbol]={'boolean': boolean, 'string': value }
...
@IF SYMBOL@         If the SYMBOL's value evaluates to true, the TEXT up
                    to @ENDIF@ delimiter is echoed to the output. Otherwise,
                    the TEXT is discarded. The branches can be nested.
