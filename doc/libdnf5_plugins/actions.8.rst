...
Each action is hooked to one specific callback. Actions for ``resolved``, ``pre_transaction`` and
...
``callback_name``
   ...
   * ``resolved`` (added in version 1.3.0)
   * ``pre_transaction``
   * ``post_transaction``
...
``package_filter``
   ...
   The filter can be non-empty only for ``resolved``, ``pre_transaction`` and ``post_transaction`` callbacks.
