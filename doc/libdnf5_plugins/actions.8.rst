...
* ``actions_vars`` - request to read action plugin variables - these variables exist only in the actions plugin context
* ``actions_attrs`` - allows getting the process ID and the version of the actions plugin
* ``packages`` - request to list of available and installed packages
...
.. code-block:: json
  ...
  {"op":"reply", "requested_op":"set", "domain":"packages", "status":"ERROR",
    "message":"<error message>"}

  {"op":"reply", "requested_op":"set", "domain":"trans_packages", "status":"ERROR",
    "message":"<error message>"}
