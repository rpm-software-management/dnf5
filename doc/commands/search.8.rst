...
given keywords from the user against various metadata. By default the command
searches all requested keys in `Name` or `Summary` fields from the RPM
package metadata.
...
``--all``
    ...
    | By applying this option the search is also relaxed by including results of any given key matches.
