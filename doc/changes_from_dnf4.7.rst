...
The differences between DNF4 and DNF5 API are not only in minor tweeks, but there are major architectural changes, therefore, it is not feasible to cover all the changes. The :ref:`tutorial_api_changes_ref-label` section describes the most common use cases and point out the main differences.
