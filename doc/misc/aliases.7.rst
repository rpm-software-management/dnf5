...
Keys:
    ...
    - ``required_values`` - Values (positional arguments) consumed by the command alias. The format is
      an array of tables; each must include either `values_help.<locale>` for the active locale or
      `values_help.C`, and may optionally include `descr.<locale>`. The default is an empty array.
