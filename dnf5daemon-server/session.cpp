...
void Session::fill_sack(bool allow_key_import) {
    if (!read_all_repos(allow_key_import)) {
        throw std::runtime_error("Cannot load repositories.");
    }
}

bool Session::read_all_repos(bool allow_key_import) {
    ...
    if (load_available_repos) {
        ...
        for (auto & repo : enabled_repos) {
            ...
            repo->set_callbacks(std::make_unique<dnf5daemon::KeyImportRepoCB>(*this, allow_key_import));
        }
