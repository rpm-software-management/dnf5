...
/// Formats the EVR of the package from a transaction. Omits the epoch when set
/// to ‹0›.
...
dnfdaemon::KeyValueMap trans_package_to_map(
    ...
    for (const auto & attr : attrs) {
        if (attr == "name") {
            dbus_pkg.emplace(attr, pkg.get_name());
        } else if (attr == "arch") {
            dbus_pkg.emplace(attr, pkg.get_arch());
        } else if (attr == "evr") {
            dbus_pkg.emplace(attr, get_evr(pkg));
        } else if (attr == "epoch") {
            dbus_pkg.emplace(attr, pkg.get_epoch());
        } else if (attr == "release") {
            dbus_pkg.emplace(attr, pkg.get_release());
        } else if (attr == "repo_id") {
            dbus_pkg.emplace(attr, pkg.get_repoid());
        } else if (attr == "action") {
            dbus_pkg.emplace(attr, libdnf5::transaction::transaction_item_action_to_string(pkg.get_action()));
        } else if (attr == "reason") {
            dbus_pkg.emplace(attr, libdnf5::transaction::transaction_item_reason_to_string(pkg.get_reason()));
        }
    }
    return dbus_pkg;
}
