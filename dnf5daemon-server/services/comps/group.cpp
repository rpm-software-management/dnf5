...
sdbus::MethodReply Group::list(sdbus::MethodCall & call) {
    ...
    if (patterns.size() > 0) {
        ...
        const auto group_with_id = key_value_map_get<bool>(options, "group_with_id", true);
        if (group_with_id) {
        ...
        const auto group_with_name = key_value_map_get<bool>(options, "group_with_name", false);
        if (group_with_name) {
