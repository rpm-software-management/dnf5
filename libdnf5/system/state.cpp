...
static void remove_new_suffix(std::filesystem::path && p, bool skip_missing) {
    ...
    auto p_new = suffix_new(std::move(p));
    ...
    try {
        ...
        if (!new_suffix_files.empty()) {
            ...
            if (std::filesystem::exists(suffix_new(get_package_state_path()))) {
                ...
                logger->warning(
                    "System state: update interruption happened during renaming which means all the new system state "
                    "files were written, using them.");
