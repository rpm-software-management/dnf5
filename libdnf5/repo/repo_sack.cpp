...
void RepoSack::Impl::update_and_load_repos(libdnf5::repo::RepoQuery & repos, bool import_keys) {
    ...
    const bool want_solv_pre_build = base->get_config().get_build_cache_option().get_value();

    // Thread pool for pre-building .solv caches in parallel.
    // Only created when build_cache is enabled; otherwise repos go straight
    // to the loader with no pool overhead.
    std::optional<utils::ThreadPool> solv_builder_pool;

    if (want_solv_pre_build) {
        solv_builder_pool.emplace(std::min<std::size_t>(std::thread::hardware_concurrency(), 8));
    }

    auto pre_build_and_send = [&](repo::Repo * repo) {
        if (!solv_builder_pool) {
    ...
    for (int run_count = 0; run_count < 2; ++run_count) {
        ...
        // Phase 1: Read metadata caches (in parallel when the pool exists,
        // serially otherwise). Each repo's read_metadata_cache is independent
        // local I/O that can safely run concurrently.
        ...
        auto check_cache = [](Repo * repo) -> CacheCheckResult {
        ...
        cache_results.reserve(repos_for_processing.size());
        ...
        if (solv_builder_pool) {
            ...
            for (auto * repo : repos_for_processing) {
                cache_futures.push_back(solv_builder_pool->submit(check_cache, repo));
            }
            ...
            for (auto * repo : repos_for_processing) {
                cache_results.push_back(check_cache(repo));
            }
        }
