...
bool copy_mode(int from, int to) {
    ...
    if (fstat(from, &stat))
        return false;
    bool success = (0 == fchmod(to, 07777 & stat.st_mode));
#ifdef HAVE_LIBACL
    // ACL
    auto acl = acl_get_fd(from);
    if (!acl) {
        // Handle file systems which do not support ACL gracefully
        success &= errno == ENOTSUP;
        return success;
    }
    success &= (0 == acl_set_fd(to, acl));
    acl_free((void *)acl);
#endif
    return success;
}
            ...
            if (check_file_fd == -1) {
                ...
                if (should_rotate(line_len)) {
                    ...
                    try {
                        ...
                        // mode, and held lock. The file will be atomically
                        // swapped with the current log file later.
                        auto new_name = fmt::format("{}.1", base_file_path.string());
                        int new_fd = ::open(new_name.c_str(), LOG_FILE_OPEN_FLAGS, LOG_FILE_OPEN_MODE);
                        if (new_fd != -1) {
