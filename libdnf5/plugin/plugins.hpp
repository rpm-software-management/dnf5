...
public:
    ...
    void resolved(const libdnf5::base::Transaction & transaction);
...
inline void Plugin::resolved(const libdnf5::base::Transaction & transaction) {
    if (iplugin_instance) {
        iplugin_instance->resolved(transaction);
    }
}
