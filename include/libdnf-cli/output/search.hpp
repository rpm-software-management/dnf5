...
/// Comparator used for sorting packages within the results.
static bool search_package_comparator(const libdnf::rpm::Package & one, const libdnf::rpm::Package & two) {
    return one.get_full_nevra() < two.get_full_nevra();
};
...
struct SearchPackages {
    std::set<libdnf::rpm::Package, decltype(&search_package_comparator)> packages{&search_package_comparator};
};
