...
void print_transaction_list(std::vector<libdnf5::transaction::Transaction> & ts_list) {
    print_transaction_list(ts_list, false);
}

void print_transaction_list(std::vector<libdnf5::transaction::Transaction> & ts_list, bool complete_history) {
    ...
    if (!ts_list.empty()) {
        ...
        id_to_actions = history.get_transaction_item_actions(ts_list, complete_history);
    }
...
void print_transaction_list_json(std::vector<libdnf5::transaction::Transaction> & ts_list) {
    print_transaction_list_json(ts_list, false);
}

void print_transaction_list_json(std::vector<libdnf5::transaction::Transaction> & ts_list, bool complete_history) {
    std::unordered_map<int64_t, int64_t> id_to_item_count;
    std::unordered_map<int64_t, std::set<libdnf5::transaction::TransactionItemAction>> id_to_actions;
    if (!ts_list.empty()) {
        libdnf5::transaction::TransactionHistory history(ts_list[0].get_base());
