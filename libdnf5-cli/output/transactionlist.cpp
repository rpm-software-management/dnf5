...
void print_transaction_list_json(std::vector<libdnf5::transaction::Transaction> & ts_list) {
    ...
    for (auto & ts : ts_list) {
        ...
        json_object_object_add(json_transaction, "id", 
            json_object_new_int64(ts.get_id()));
        json_object_object_add(json_transaction, "command_line", 
            json_object_new_string(ts.get_description().c_str()));
        json_object_object_add(json_transaction, "start_time", 
            json_object_new_string(libdnf5::utils::string::format_epoch(ts.get_dt_start()).c_str()));
        json_object_object_add(json_transaction, "end_time", 
            json_object_new_string(libdnf5::utils::string::format_epoch(ts.get_dt_end()).c_str()));
        json_object_object_add(json_transaction, "user_id", 
            json_object_new_int64(ts.get_user_id()));
        ...
        json_object_object_add(json_transaction, "releasever", 
            json_object_new_string(ts.get_releasever().c_str()));
        json_object_object_add(json_transaction, "altered_count", 
            json_object_new_int64(id_to_item_count.at(ts.get_id())));
