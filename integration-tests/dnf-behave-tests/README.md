...
dnf-behave-tests is the integration test suite for DNF5. It contains the `dnf`
test suite.

The test suite's directory structure is that of a behave test suite. It
contains .feature files, which contain the test scenarios, defined in steps. The
implementation of the steps is in the `steps` directory. Next to it is also the
`environment.py` file, defining the global setup of the suite. Common steps and
utility code are located in the `common` directory.


Generating Test Data
--------------------

To run the tests, you first need to generate the test data:
```
