Feature: distro-sync with --from-repo
...
Scenario: distro-sync pkgB from "from-repo-updates1" and install new dependency from another repo
  Given I use repository "from-repo"
    And I use repository "from-repo-updates1"
    And I use repository "from-repo-updates2"
   When I execute dnf with args "distro-sync --from-repo=from-repo-updates1 pkgB"
   Then the exit code is 0
    And Transaction is following
        | Action        | Package                                   |
...
Scenario: distro-sync all RPMs "from-repo-updates1" and install new dependency from another repo
  Given I use repository "from-repo"
    And I use repository "from-repo-updates1"
    And I use repository "from-repo-updates2"
   When I execute dnf with args "distro-sync --from-repo=from-repo-updates1 '*'"
   Then the exit code is 0
    And Transaction is following
        | Action        | Package                                   |
        | downgrade     | pkgB-0:1.1-1.fc29.x86_64                  |
