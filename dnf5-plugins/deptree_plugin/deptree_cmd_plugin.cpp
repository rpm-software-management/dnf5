...
constexpr const char * PLUGIN_NAME{"deptree"};
...
constexpr const char * attrs_value[]{"Marek Blaha", "mblaha@redhat.com", "Display package dependency trees."};

class DeptreeCmdPlugin : public IPlugin {
public:
...
std::vector<std::unique_ptr<Command>> DeptreeCmdPlugin::create_commands() {
    std::vector<std::unique_ptr<Command>> commands;
    commands.push_back(std::make_unique<DeptreeCommand>(get_context()));
    return commands;
}
...
IPlugin * dnf5_plugin_new_instance([[maybe_unused]] ApplicationVersion application_version, Context & context) try {
    return new DeptreeCmdPlugin(context);
} catch (...) {
