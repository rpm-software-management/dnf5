...
void ManifestCommand::set_argument_parser() {
    ...
    // Configure 'manifest' command in the Parser
    //
    ...
    // Link the option to the ManifestCommand's class member.
    foo->link_value(foo_option);

    // Register the '--foo' argument to the 'manifest' command.
    this_cmd.register_named_arg(foo);
}

void ManifestCommand::run() {
    // The behavior of the command is a simple hello world.
    std::cout << "Manifest Command" << std::endl;
