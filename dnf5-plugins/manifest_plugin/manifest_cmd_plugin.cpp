...
constexpr const char * PLUGIN_NAME{"manifest"};
...
constexpr const char * attrs_value[]{
    "Evan Goode", "egoode@redhat.com", "Operations for working with RPM package manifest files"};

class ManifestCmdPlugin : public IPlugin {
public:
...
std::vector<std::unique_ptr<Command>> ManifestCmdPlugin::create_commands() {
    std::vector<std::unique_ptr<Command>> commands;
    commands.push_back(std::make_unique<ManifestCommand>(get_context()));
    return commands;
}
...
IPlugin * dnf5_plugin_new_instance([[maybe_unused]] ApplicationVersion application_version, Context & context) try {
    return new ManifestCmdPlugin(context);
} catch (...) {
