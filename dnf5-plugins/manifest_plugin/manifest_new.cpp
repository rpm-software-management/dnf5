...
void ManifestNewCommand::set_argument_parser() {
    ...
    arches_option = dynamic_cast<libdnf5::OptionStringList *>(
        parser.add_init_value(std::make_unique<libdnf5::OptionStringList>(std::vector<std::string>())));
    auto * arches_arg = parser.add_new_named_arg("arches");
    arches_arg->set_long_name("arches");
    arches_arg->set_description("Explicitly specify basearches to use");
    arches_arg->set_has_value(true);
    arches_arg->set_arg_value_help("<ARCH>,...");
    arches_arg->link_value(arches_option);
...
void ManifestNewCommand::configure() {
    ...
    } else if (arches_option->get_priority() > libdnf5::Option::Priority::DEFAULT) {
        arches = std::vector<std::string>(arches_option->get_value().begin(), arches_option->get_value().end());
    } else {
