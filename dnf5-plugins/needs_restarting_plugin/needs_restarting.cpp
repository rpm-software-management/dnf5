...
const sdbus::ServiceName SYSTEMD_DESTINATION_NAME{"org.freedesktop.systemd1"};
...
void NeedsRestartingCommand::processes_need_restarting(Context & ctx, bool exclude_services) {
    ...
    // Also check reverse dependencies: if any process depends on an updated package,
    // it needs to be restarted too
    for (const auto & [exe_path, process_info] : running_processes) {
        if (processes_needing_restart.find(exe_path) != processes_needing_restart.end()) {
            // Already marked as needing restart
            continue;
        }

        // Check if this process depends on any updated package
        const auto & deps = recursive_dependencies(process_info.package, installed);
        for (const auto & dep : deps) {
            if (updated_packages.contains(dep)) {
                processes_needing_restart.insert(exe_path);
                break;
            }
        }
