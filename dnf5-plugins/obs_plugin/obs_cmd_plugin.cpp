...
constexpr const char * PLUGIN_NAME{"obs"};
...
class ObsCmdPlugin : public IPlugin {
public:
    ...
    const char * get_attribute(const char * attribute) const noexcept override {
        for (size_t i = 0; attrs[i]; ++i) {
            ...
            if (std::strcmp(attribute, "description") == 0) {
                return OBS_COMMAND_DESCRIPTION;
            }
        }
        return nullptr;
    }
    ...
    std::vector<std::unique_ptr<Command>> create_commands() override {
        ...
        commands.push_back(std::make_unique<ObsCommand>(get_context()));
        return commands;
    }
...
IPlugin * dnf5_plugin_new_instance([[maybe_unused]] ApplicationVersion application_version, Context & context) try {
    return new ObsCmdPlugin(context);
} catch (...) {
