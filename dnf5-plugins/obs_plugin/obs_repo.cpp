...
static void parse_project_repo_spec(
    ...
    /// Convert from project_repo_spec to hubspec, project, and reponame
    /// Example:
    ...
    if (hubspec) {
        ...
        *hubspec = config->get_hub_hostname(match[HUBSPEC]);
    }
    if (project)
...
static void parse_repo_id(
    ...
    std::string * project = nullptr,
    std::string * reponame = nullptr) {
    /// Convert from obs repo id to hubspec, project, and reponame
    /// Example:
...
static std::string build_repo_id(
    ...
    return "obs:" + hubspec + ":" + project + ":" + reponame;
}
...
ObsRepo::ObsRepo(
    ...
    parse_project_repo_spec(base, project_repo_spec, &hubspec, &project, &reponame);
    ...
    auto html_url = config->get_repo_url(hubspec, project, reponame);
    auto html = std::make_unique<Html>(base, html_url);
    auto download_url = html->get_download_url();
    if (download_url == "")
        throw std::runtime_error(libdnf5::utils::sformat(_("Repository '{}' does not exist or has no published packages"), project_repo_spec));

    auto repo_file_url = download_url + "/" + project + ".repo";
...
ObsRepo::ObsRepo(
    ...
    parse_repo_id(my_repo_id, &hubspec, &project, &reponame);
}
...
std::string ObsRepo::get_id() const {
    return build_repo_id(hubspec, project, reponame);
}
...
static void warn_weird_obs_repo(libdnf5::repo::RepoWeakPtr dnfRepo) {
    ...
    std::string repo_file_path = dnfRepo->get_repo_file_path();
    if (!repo_id.starts_with("obs:")) {
        warning(_("Repo id '{}' in '{}' is suspicious"), repo_id.c_str(), repo_file_path.c_str());
        return;
    }

    // Check that the repo filename matches the repo ID
    std::string expected_filename = "_" + repo_id + ".repo";
    std::filesystem::path repo_path(repo_file_path);
    if (expected_filename != repo_path.filename().string())
        warning(_("Repo id '{}' doesn't belong to '{}' repofile"), repo_id.c_str(), repo_file_path.c_str());
}
...
void installed_obs_repositories(libdnf5::Base & base, ObsRepoCallback cb) {
    ...
    for (const auto & repo : query.get_data()) {
        std::filesystem::path repo_file_path = repo->get_repo_file_path();

        if (!std::filesystem::exists(repo_file_path) || !is_obs_repofile(repo_file_path))
            continue;

        warn_weird_obs_repo(repo);
        obs_repos.emplace(repo_file_path.string(), ObsRepo(base, repo_file_path));
    }
...
static std::string repo_id_from_project_repo_spec(libdnf5::Base & base, const std::string & project_repo_spec_string) {
    std::string hubspec, project, reponame;
    parse_project_repo_spec(base, project_repo_spec_string, &hubspec, &project, &reponame);
    return build_repo_id(hubspec, project, reponame);
}
