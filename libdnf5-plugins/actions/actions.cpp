...
public:
    ...
    void resolved(const libdnf5::base::Transaction & transaction) override {
        current_hook = Hooks::RESOLVED;
...
void Actions::parse_action_files() {
    ...
    for (const auto & path : action_paths) {
        ...
        while (std::getline(action_file, line)) {
            ...
            if (hook != Hooks::RESOLVED && hook != Hooks::PRE_TRANS && hook != Hooks::POST_TRANS) {
                if (!pkg_filter.empty()) {
                    throw ActionsPluginError(
                        M_("Error in file \"{}\" on line {}: Package filter can only be used in RESOLVED, PRE_TRANS and "
                           "POST_TRANS hooks"),
                        path.native(),
                        line_number);
                }
    ...
    try {
        ...
        if (op == "get") {
            ...
            if (domain == "conf") {
                ...
                if (domain == "trans_packages" && current_hook != Hooks::RESOLVED && current_hook != Hooks::PRE_TRANS &&
                    current_hook != Hooks::POST_TRANS) {
                    throw JsonRequestError(
                        "Domain \"trans_packages\" used outside the hooks \"resolved\", \"pre_transaction\" and "
                        "\"post_transaction\"");
                }
                auto * jargs = get_object(request, "args");
