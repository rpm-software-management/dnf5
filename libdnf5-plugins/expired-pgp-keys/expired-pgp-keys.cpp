            ...
            for (auto & key_info : rpm_signature.parse_key_file(key_url)) {
                if (rpm_signature.key_present(key_info)) {
                    ...
                    if (key_timestamp > 0 && key_timestamp < current_timestamp) {
                        ...
                        if (callbacks && !callbacks->repokey_remove(expired_key_info)) {
                            continue;
                        }
                        logger.debug("Expired PGP Keys Plugin: Removing the 0x{} key (user id: \"{}\").", 
                                     key_info.get_short_key_id(),
                                     !user_ids.empty() ? user_ids.front() : "");
                        ...
                        if (!removed) {
                            ...
                            callbacks->repokey_removed(expired_key_info);
                        }
                    }
                }
            }
